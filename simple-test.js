...
async function test() {
  // Load simplified IDL from file
  const idl = JSON.parse(fs.readFileSync('./lib/solana/simple-ajo-idl.json', 'utf-8'));
  ...
  // Create program with simplified IDL
  ...
  try {
    ...
    const tx = await program.methods
      .createPool(
        ...
        { sol: {} },  // lowercase for normalized IDL
        new anchor.BN(contributionAmountLamports),
        5, // totalMembers
        10, // totalCycles
        { monthly: {} },  // lowercase for normalized IDL
        1, // creatorPosition
        poolBump
      )
      .accounts({
