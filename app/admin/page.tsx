...
import { getUserPools } from '@/lib/pools/actions'
...
export default function AdminPage() {
  ...
  const fetchPools = useCallback(async () => {
    ...
    try {
      const userPools = await getUserPools(publicKey.toBase58())
      setPools(userPools)
    } catch (err) {
      setPools([])
    } finally {
  ...
  }, [connected, publicKey, fetchPools])

  const createdPools = pools.filter((pool) => pool.creator === publicKey?.toBase58())
  ...
  if (!connected || !publicKey) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center">
        ...
        <p className="mt-4 text-lg">Please connect your wallet to view your admin dashboard.</p>
      </div>
    )
  }
  return (
